...
storage.marc_win_loc_x = 0
storage.marc_win_loc_y = 176 -- puts us just below rocket stats
-- ----------------------------------------------------------------
...
local function get_item_or_fluid(name)
    ...
    if proto == nil
    then
        debug_log(__FUNC__(), name .. " has no prototype")
...
local function get_proto(name)
    ...
    then
        ...
        p = prototypes.item[name]
        itype = "item"
    else
        itype = "fluid"
        p = prototypes.fluid[name]
    end
    if(p == nil)
    then
...
function destroy_marc_gui(player)
    ...
    storage.marc_win_loc_x = root.marc_gui_top.location.x
    storage.marc_win_loc_y = root.marc_gui_top.location.y
    root.marc_gui_top.destroy()
end
-- ----------------------------------------------------------------

local function init_modeffects()
    return { speed = 0, prod = 0, consumption = 0, pollution = 0, quality = 0 }
end
...
local function find_belts()
    ...
    debug_print(__FUNC__(), "type " .. entity_proto.type)
        ...
        then
            ...
            local maxh = entity_proto.get_max_health()
...
local function scale_rate(player, name, count)
    ...
    elseif unit_type == "wagon"
        then
            ...
            then
                cargo_wagon_proto = prototypes.entity["cargo-wagon"]
                total_capacity = cargo_wagon_proto.get_inventory_size(1) * proto.stack_size
            else
                fluid_wagon_proto = prototypes.entity["fluid-wagon"]
...
local function write_marc_gui(player, inout_data)
    ...
    if( storage.marc_win_loc_x == nil )
    then
        storage.marc_win_loc_x = 0
    end
    if( storage.marc_win_loc_y == nil )
    then
        storage.marc_win_loc_y = 172
    end
    root.marc_gui_top.location = { storage.marc_win_loc_x, storage.marc_win_loc_y }
    ...
    local ix = storage.marc_selected_units[player.index]
    debug_print("storage.marc_selected_units[" .. player.index .. "] = " .. ix)
...
-- debug_print("calc_mod help " .. prototypes.item[modname])
...
local function calc_mods(entity, modeffects, effectivity, machine_proto)
    ...
    for _,item_count_and_quality in pairs(modcontents)
    do
        modname = item_count_and_quality.name
        modquant = item_count_and_quality.count
        modqual = item_count_and_quality.quality

        debug_print("calc_mods proto=" .. prototypes.item[modname].name)
        debug_print("calc_mods modname=" .. modname .. ", modquant=" .. modquant)
...
local function is_machine_in_range_of_beacon(entity, beacon)
    ...
    local beac_dist = prototypes.entity[beacon.name].get_supply_area_distance()
...
local function check_beacons(surface, entity)
    ...
    local machine_proto = prototypes.entity[entity.name]
    ...
    for _, entity_proto in pairs(prototypes.get_entity_filtered(
        ...
        do            
            ...
            then
                ...
                --local distance = prototypes.item[entity_proto.name].get_supply_area_distance()
...
local function calc_assembler(entity, inout_data, beacon_modeffects)
    ...
    then
        ...
        then
            ...
            fuel_proto = prototypes.item[fuel_name]
...
local function calc_mining(inout_data, surface, player, entity)
    ...
    local beacon_modeffects = { speed = 0, prod = 0, quality = 0 }
...
local function calc_inserter(inout_data, surface, entity)
    ...
    then
            ...
            local rot_speed = prodproto.get_inserter_rotation_speed()
            local energy_per_rot = 1 --prodproto.energy_usage
            local ext_speed = prodproto.get_inserter_extension_speed()
...
local function getFluidProduction(proto)
      ...
      local fluid_prototype = prototypes.fluid[fluid_filter]   
...
local function calc_generator(inout_data, surface, entity)
    ...
    local genProto = prototypes.entity[entity.name]
...
script.on_event(defines.events.on_player_selected_area,
    function(event)
        ...
        storage.marc_selected_unit_index = storage.marc_selected_unit_index or {}
        local player = game.players[event.player_index]
        ...
        storage.marc_inout_data_by_player = storage.marc_inout_data_by_player or {}
        storage.marc_inout_data_by_player[event.player_index] = inout_data
        storage.marc_inout_data = inout_data
...
local function on_hotkey_main(event)
    ...
    storage.marc_hint = storage.marc_hint or 0    
    if storage.marc_hint == 0
    then
...
local function on_gui_click(event)
    ...
    if storage.marc_inout_data_by_player ~= nil
    then
        local inout_data = storage.marc_inout_data_by_player[event.player_index]
        if inout_data ~= nil
        then
...
local function on_gui_selection(event)
    ...
    then
        ...
        then
            open_gui(event, storage.marc_inout_data)
        else
            open_gui(event, storage.marc_inout_data_by_player[event.player_index])
        end
    end
end
...
if event.parameter == "debug"
    then
        storage.marc_debug = true
        debug_print("marc debugging is on")
    elseif event.parameter == "nodebug"
    then
        debug_print("marc debugging is off")
        storage.marc_debug = false
    elseif event.parameter == "showloc"
    then
        if storage.marc_win_loc_x ~= nil
        then
            debug_log("event", "marc location x " .. storage.marc_win_loc_x )
        end
        if storage.marc_win_loc_y ~= nil
        then
            debug_log("event", "marc location y " .. storage.marc_win_loc_y)
        end
    elseif event.parameter == "resetloc"
    then
        storage.marc_win_loc_x = 0
        storage.marc_win_loc_y =  176
    else
